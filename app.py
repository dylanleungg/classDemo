...
spx_returns['CUM RETURN'] = (1 + spx_returns['SPX INDEX RETURN']) * ((1 + spx_returns['SPX INDEX RETURN']).shift(1)) - 1
...
long_only_returns['LONG CUM RETURN'] = ((1 + long_only_returns['LONG PORTFOLIO RETURN']) * (1 + long_only_returns['LONG PORTFOLIO RETURN']).shift(1)) - 1
ls_returns['LS CUM RETURN'] = ((1 + ls_returns['LS PORTFOLIO RETURN']) * (1 + ls_returns['LS PORTFOLIO RETURN']).shift(1)) - 1
equal_weight['EQW CUM RETURN'] = ((1 + equal_weight['EQW PORTFOLIO RETURN']) * (1 + equal_weight['EQW PORTFOLIO RETURN']).shift(1)) - 1
...
st.write('You selected', ticker_select)

start = st.date_input('Start Date', share_prices.index)
end = st.date_input('End Date', share_prices.index)

fig_one = px.line(share_prices.loc[start:end], title = 'Share Price Performance Over Time')
st.plotly_chart(fig_one)

fig_two = px.line(finalDF.loc[start:end][['LONG CUM RETURN', 'LS CUM RETURN', 'EQW CUM RETURN']], title = 'Cumulative Returns')
...
portfolio_select = st.select('Select a Portfolio', ['LONG', 'LS', 'EQW'])
