...
returns_data = pd.read_csv('returns_data.csv', index_col = ['Unnamed: 0'])
...
for stock in stocks:
    share_prices[stock + ' CUM RETURN'] = (1 + returns_data[stock]).cumprod() - 1
...
start = st.date_input('Pick Your Start Date')
...
fig_two = px.line(finalDF.loc[start:end][['LONG CUM RETURN', 'LS CUM RETURN', 'EQW CUM RETURN']], title = 'Portfolio Cumulative Returns')
